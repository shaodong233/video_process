...
def gen_gauss_window(lw, sigma):
    ...
    return g
# wsd add
def eigen_calculation(height, width, upper_left, off_diag, lower_right):
    Eigens = np.zeros((height, width, 2))
    trace = upper_left + lower_right
    det = upper_left * lower_right - off_diag**2
    temp = np.zeros_like(trace)

    valid_indices = (trace**2 - 4 * det) >= 0
    temp[valid_indices] = np.sqrt(trace[valid_indices]**2 - 4 * det[valid_indices])

    Eigens[:, :, 0] = (trace + temp) / 2
    Eigens[:, :, 1] = (trace - temp) / 2
    return Eigens
# wsd 
def motion_feature_extraction_frame(frames):
    ...
    mode10x10 = np.bincount(motion_amplitude.flatten().astype(int)).argmax()
...
def NSS_spectral_ratios_feature_extraction_frame(frames):
    ...
    dct_diff5x5 = np.zeros((1,int(frames.shape[1]/mblock), int(frames.shape[2]/mblock),mblock**2), dtype=np.float32)
