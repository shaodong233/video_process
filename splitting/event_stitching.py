...
# 获取 ImageBind 目录的绝对路径
imagebind_dir = os.path.join(current_dir, 'ImageBind')

# 将 ImageBind 目录添加到 sys.path 中
...
if __name__ == "__main__":
    ...
    for video_path in tqdm(video_paths):
        ...
        events_raw, event_feature_raw = cutscene_stitching(cutscenes, cutscene_feature, eventcut_threshold=args.eventcut_threshold) # eventcut_threshold值越小，diff>eventcut_threshold越成立，也就是越容易开始一个新的场景，即分的越开
        ...
        video_events[video_path.split("/")[-1]] = transfer_timecode(events, fps) # 设置单个剪辑场景最长 1 min
